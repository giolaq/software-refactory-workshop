...
| Review | Fresh verifier plus optional adversarial critic; PR-level verification | Read-only Code Review role, comment-to-rework loop, exact-head approval, Supervisor merge | Ours for closed-loop repair; Addy's for stronger skeptical verification |
...
Our Lean profile retains a human merge boundary, but Standard and Assured allow the Code Review role to approve and the Supervisor to authorize merge. The orchestrator rechecks the PR head and calls GitHub (`factory/factory_contracts.py`, lines 13–57; `factory/orchestrator.py`, lines 1365–1437). This is sophisticated automated merge safety, but it is still automated merge. In a one-login workshop, GitHub may record the agent decision only as a comment rather than a formal approval (`factory/github_backend.py`, lines 204–235).
