...
class FactoryContractTests(unittest.TestCase):
    ...
    def test_cli_exposes_frozen_workshop_version(self):
        ...
        self.assertEqual(result.stdout.strip(), "factory workshop-v1.2.2")
    ...
    def test_release_audit_rejects_machine_local_links_in_docs(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (repo / "factory/factory_contracts.py").write_text(
                'WORKSHOP_VERSION = "workshop-v1.2.0"\n',
            )
            (repo / "factory/WORKSHOP_OUTLINE.md").write_text("# workshop-v1.2.0\n")
            (repo / "factory/FACILITATOR.md").write_text("# workshop-v1.2.0\n")
            (repo / "workshop-guide/app/page.tsx").write_text("workshop-v1.2.0\n")
    ...
    def test_release_check_help_exposes_live_agent_selection(self):
        ...
        self.assertIn("--live-agent {bedrock,claude,codex,cursor}", result.stdout)
